...
def _invite_users(db: Session, thread: Thread, host: User, user_ids: list[int]) -> list[dict[str, Any]]:
    ...
    for user_id in unique_ids:
        ...
        _notify_thread_user(
            ...
            body=f"{host.name} invited you to {_notification_title_snippet(thread.title)} — you in?",
            thread=thread,
            actor=host,
        )
    return invited
...
def edit_thread(
    ...
    _notify_members(
        ...
        body=f"{current_user.name} made changes to {_notification_title_snippet(thread.title)}.",
        ...
        if next_host:
            ...
            if promoted:
                _notify_thread_user(
                    ...
                    title="👑 You're hosting now",
                    body=f"You've been promoted to host of {_notification_title_snippet(thread.title)}.",
                    thread=thread,
                    actor=current_user,
                )
        else:
